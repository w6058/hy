		...
		if(IS_POST){
			...
			if(I("post.x_wid")){
				...
				if(I('post.cc_status') == "-1"){
					$url = __ROOT__."/index.php/admin/Client/messages/case/cao/messages/".$id;
				}elseif (I('post.cc_status') == "1"){
					$url = __ROOT__."/index.php/admin/Client/messages/case/dai/messages/".$id;
				}
				$type = "update";
				...
				$url = __ROOT__."/index.php/admin/Client/forms";
				$type = "insert";
...
//									$M = Mobile($result_arr["s_phone"],'亲爱的：'.$result_arr["s_name"].'同事。'.$result_arr["u_name"].'这位客户已提交新工单，工单标题为：“'.$data['title'].'”。请及时查看，并且处理。');
	...
	public function messages(){
		...
		if($limits == 3){
			...
			$wid = rtrim($wid,",");
            $url = __ROOT__."/index.php/admin/Console/dashboard";
            if ($wid==''){
                echo "<script>alert('你暂时还没有负责的客户。'); location.href='$url';</script>";
                exit;
            }
			//显示当前售后人员所负责 的 客户工单
			$list = $this->sel_sql("work","wc_sataus='$sta_nb' and uid in($wid) $where","puddate desc");
			$this->assign('list',$list);
			
		}else if($limits == 1){
			
			$list = $this->sel_sql("work","wc_sataus='$sta_nb' $where","puddate desc");
			$this->assign('list',$list);
		...
		//处理对话操作
		if(IS_POST){
			$url = __ROOT__."/index.php/admin/Client/messages/case/zhong/messages/".I('post.pid');
...
//								$M = Mobile($main["s_phone"],'亲爱的：'.$main['s_uname'].'同事。'.$main['u_uname'].'这位客户的工单标题为：“'.$main['w_title'].'”已有最新追加。请及时查看，并且处理。');
		...
		if(I("get.type") == "chu" && $limits == "2"){
			...
			$url = __ROOT__."/index.php/admin/Client/messages/case/dai";
			$data	=	array(
					"wc_sataus"		=>	2,
					"tz_status"		=>	1,
			);
...
//				$M = Mobile($main["u_phone"],'尊敬的客户：'.$main['u_uname'].'。您好！您的工单标题为：“'.$main['w_title'].'”。正在处理中，请耐心等候。');
		...
		if(I("get.type") == "wang" and $limits == "3"){
			...
			$url = __ROOT__."/index.php/admin/Client/messages/case/zhong";
...
//				$M1 = Mobile($main["u_phone"],'尊敬的客户：'.$main['u_uname'].'。您好！您的工单标题为：“'.$main['w_title'].'”已取消。若不是本人操作，请联系售后人员。');
		...
		$data = array(
				...
				'url'			=>	__ROOT__."/index.php/admin/Client/messages/case/".$status,
				'sou'			=>	'&sou='.I("get.sou"),
		);
		$this->assign('data',$data);
