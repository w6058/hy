...
insert into t_shop(s_name,s_username,s_password,s_card,s_phone,s_address,s_email,s_num,s_license) values('绵阳1点','shop','111111','510722199302544875','12430013703','四川绵阳','anasdfdfassadf@163.com',9999999,12132423);
insert into t_shop(s_name,s_username,s_password,s_card,s_phone,s_address,s_email,s_num,s_license) values('花荄1点','shop1','111111','5107221993430254475','12430013703','四川绵阳','anasdfdfassadf@163.com',9999999,12132423);
	#商铺管理员表
...
create table t_member(
	id int primary key auto_increment,
	m_card varchar(32) not null comment '会员卡号',
	...
	m_num int not null default 0 comment'用户总金额',
	m_fenqi int default null comment'分期金额',
	m_day int default null comment'金额分期天数',
	m_status int default null comment'是否发卡',
	create_at TIMESTAMP default now() comment'会员账号创建时间',
	paytime TIMESTAMP default CURRENT_TIMESTAMP comment'最后一次充值时间'
);
create table t_member_limit(
	...
	m_limit int default 10000 comment'每天限定使用的金额',
...
create table t_record(
	...
	s_id int not null,
	m_id int not null,
	grade int comment 'vip用户当次购买的积分或者消费积分',
	gradetype enum('Y','N'),
